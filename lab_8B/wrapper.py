...
def test_continued_evaluations(inps):
    ...
    for i in inps:
        ...
        if out['ok']:
            env = out['output'][1]
        if out['ok']:
            if isinstance(out['output'][0], (int, float, lab.LinkedList)):
                out['output'] = list_from_ll(out['output'][0])
            else:
...
def list_from_ll(ll):
    if isinstance(ll, lab.LinkedList):
        if ll.next is None:
            return [list_from_ll(ll.elt)]
        return [list_from_ll(ll.elt)] + list_from_ll(ll.next)
    else:
        return ll
